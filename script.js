...
function Appear_MusicPlayer() {
    ...
    // Prevent further clicks if the button was already clicked within 0.5s
    if (buttonClicked) return;

    buttonClicked = true; // Disable further clicks for 0.5s

    setTimeout(function() {
        buttonClicked = false; // Re-enable the button click after 0.5s
...
document.addEventListener('DOMContentLoaded', () => {
    const musicList = [
      ...
      { title: "Rewrite the stars", file: "backup/file_music/Rewrite_the_stars.mp3" },
    ...
    function playTrack(index) {
      ...
      document.querySelector(`.music-item[data-index="${index}"]`).classList.add('playing');
      nowPlayingElement.textContent = `Playing: ${track.title}`;
      currentAudio.play().catch(e => {
        console.log("Audio playback failed. This is expected in this demo since we don't have actual audio files.");
      });
      isPlaying = true;
      updatePlayIcon();
    }
